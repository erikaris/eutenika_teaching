# Materials for teaching at UMM - Eutenika
# BY: Erika Siregar

#======== Columns Explanation ========================================

# https://github.com/erikaris/eutenika_teaching/blob/master/hotel_booking_dict.md

# =========== installing the libraries ====================================

install.packages(c("tidyverse","readr", "dplyr", "ggplot2", "countrycode", "gtools", "plotrix", "corrplot", "GGally"))
...
names(hb)
# contingency/frequency table of certain fields
table(hb$hotel)
table(hb$hotel, hb$is_canceled)
table(hb$hotel, hb$is_canceled, hb$arrival_date_year)
...
guest1 <- summarize(hb_clean, sum(guests))
total_guests <- as.numeric(guest1)
...
hb_arpt <- hb_clean %>% 
            ...
            ggplot(aes(x = reserved_room_type, y = mean_rp, fill = hotel)) +
              ...
              scale_y_continuous(name = "average rate per night per person", limits = c(0, 120), breaks = seq(0, 120, 10)) # + facet_wrap(~hotel)  # fancy to facet the graph?
...
hb_num <- hb_clean_cancel %>% 
            ...
            select(-c(is_canceled, is_repeated_guest, arrival_date_year, arrival_date_day_of_month, company, agent, arrival_date_week_number))
...
# takes more time than corrplot if we run the whole fields at once. 
# so, select several fields that are deemed important. 

hb_num %>% 
  select(c(lead_time, days_in_waiting_list, previous_cancellations, stays_in_week_nights, stays_in_weekend_nights)) %>% 
  ggpairs(hb_num)
...
sapply(hb_clean_cancel, unique)

# checking with boxplot. 
# example of boxplot of hotel and lead_time

hb_clean_cancel %>% 
  ggplot(aes(x = hotel, y = lead_time, fill = factor(is_canceled))) +
...
# try creating boxplot of other variables. 


# =============16. basic desc stat =======================

summary(hb_clean_cancel)

sapply(hb_clean_cancel, mean)

sapply(hb_clean_cancel, sd)
